...
export function get_fee( quantity: Asset, settings: Settings | { pool_fee: number; stability_fee: number }  ): Asset {
...
export function get_inverse_fee( out: Asset, settings: Settings | { pool_fee: number; stability_fee: number } ): Asset {
    const { amount, symbol } = out;
