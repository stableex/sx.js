...
(async () => {
    ...
    const EOS = symbol_code("EOS")

    const quantity1 = new Asset(10000, symbol("EOS", 4))
    const quantity2 = new Asset(100000, symbol("EOS", 4))
    const quantity3 = new Asset(1000000, symbol("EOS", 4))

    for ( const amplifier of [1, 3, 5, 10, 20, 50] ) {
        pools["EOS"].amplifier = amplifier;
        console.log("\namplifier =", amplifier);
